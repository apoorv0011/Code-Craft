...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (!isSupabaseConfigured()) {
        setError("Signup is disabled. Supabase has not been configured by the developer.");
        return;
    }
  ...
  return (
    <div className="relative flex items-center justify-center h-screen px-4 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 right-1/4 w-[500px] h-[500px] bg-blue-500/15 rounded-full blur-orb animate-pulse"></div>
        <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-indigo-500/15 rounded-full blur-orb animate-pulse" style={{animationDelay: '2s'}}></div>
      </div>
      ...
      >
        ...
        <div className="text-center mb-5">
          <h1 className="text-3xl md:text-4xl font-bold mb-1 gradient-text">Create Account</h1>
          <p className="text-gray-400 text-sm font-light">Join CodeCraft today</p>
        </div>
        
        {/* Form Card */}
        <form
          onSubmit={handleSignup}
          className="glass-strong border border-white/10 rounded-2xl p-5 space-y-3.5 shadow-2xl backdrop-blur-xl"
        >
          {/* Email Field */}
          <div>
